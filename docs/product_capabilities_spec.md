...
| `BodyLog` | `log_id, user_id, date, weight_kg, intake_kcal, intake_is_real, steps` | Raw weekly weight/nutrition/steps record. `intake_is_real` distinguishes logged vs. assumed intake. | Implemented (`data/db/BodyLogDAO.py`). Phase 9.1: `waist_cm`/`neck_cm` moved off this table into `BodyMeasurement` below. |
| `BodyMeasurement` | `measurement_id, user_id, date, waist_cm, neck_cm` (+ nine Phase 9.3 record-only fields: `shoulder_cm, chest_cm, hips_cm, biceps_r_cm, biceps_l_cm, thigh_r_cm, thigh_l_cm, calf_r_cm, calf_l_cm`) | Sporadically-logged perimeters, decoupled from `BodyLog`'s cadence — "static until next update" resolution (`BodyMeasurementManager.get_effective`) supplies `waist_cm`/`neck_cm` to the engine for any week without its own reading. | Implemented (Phase 9.1/9.3: `data/db/BodyMeasurementDAO.py`, `services/BodyMeasurementManager.py`, `GET`/`POST /api/body-measurements`, `PUT`/`DELETE /api/body-measurements/<id>`). |
...
Everything above (§14–§16) comes from the original "Demo_cut" (cut/deficit)
spec and is implemented. This section records a **second** source
document — a bulk/volume ("Demo_bulk") profile verified against a separate
...
Demo_cut's (TEF as a multiplier, a different weight basis for the surplus),
...
| `GoalPlan` | `direction` derived from `weekly_rate`'s sign (no new column needed, unless a UI wants to store the user's intent independent of the numeric rate) — **superseded, Phase 12.1 (done, v6.0.0)**: `direction` is now a real, explicitly-chosen `goal_plans` column (migration `m0005`), set when the goal is created rather than inferred; see `docs/composition_spec.md`'s "Phase 12" section. | F1: cut vs. bulk labeling. |
...
- **One shared formula, not a fork**: the source doc's literal TDEE/
  target-calorie formulas look structurally different from Demo_cut's (TEF as
  ...
  bug to chase.
- **Defaults preserve Demo_cut exactly**: `fat_offset=0.0`, `ffmi_coef=6.3`,
  and `bf_weight_rfm/navy/deurenberg=0.50/0.25/0.25` are the new defaults,
  not Demo_bulk's calibrated `0.02`/`6.1` — a user with no override must see
  identical numbers to today. Same "no row = today's behavior" contract as
  every other `EngineSettings` field. Demo_bulk's `0.02`/`6.1` are one
